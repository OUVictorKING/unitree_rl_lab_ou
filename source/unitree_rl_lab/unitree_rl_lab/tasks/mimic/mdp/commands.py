...
class MotionCommand(CommandTerm):
    ...
    # 如果均匀采样，容易片段被反复学会，难片段一直学不会，因为出现频率不够，因此需要自适应采样
    ...
    # 将锚点坐标复制到每个身体部位，便于后续进行广播运算，比如计算每个锚点到每个身体部位的距离或偏移量
    # 防止机器人为了拿到高 reward ，只会跟踪anchor的绝对位置
    def _update_command(self):
        ...
        # 读取当前机器人的x和y坐标，并且希望保持
        delta_pos_w = robot_anchor_pos_w_repeat
        # 但是z方向上的高度希望达到标准动作的高度（因为这可能关乎到下蹲，上跳等动作）
        delta_pos_w[..., 2] = anchor_pos_w_repeat[..., 2]
        # 只取 yaw 对齐，计算与标准 yaw 的偏差（朝向很重要！）
        ...
        # 失败多的时间段会被赋予更高采样概率（和adaptive sampling相关）
        self.bin_failed_count = (

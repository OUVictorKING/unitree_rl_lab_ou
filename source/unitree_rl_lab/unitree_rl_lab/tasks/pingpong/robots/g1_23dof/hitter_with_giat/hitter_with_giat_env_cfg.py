...
class CurriculumCfg:
    ...
    pingpong = CurrTerm(
        ...
        params={
            ...
            # Locomotion-First curriculum: y unlocks early so the policy learns
            # to MOVE to far balls before timing precision is tightened. This
            # replaces the legacy v→y staging when set True.
            "locomotion_first": True,
            # Stage 1 (y) unlock: ONLY hsr_ema gate. Lower than legacy 0.80 so
            # the policy starts widening hit_y_world_cap as soon as it can hit
            # near balls reliably.
            "y_unlock_hsr": 0.65,
            # Stage 2 (v) unlock: y has reached cap_max AND hsr holds AND
            # cos_sim is consistent. Effective only in locomotion_first mode.
            "v_unlock_hsr": 0.70,
            "v_unlock_cos_sim": 0.55,
            # Stage 3 (precision) unlock: ball speed has reached 4 m/s AND
            # hsr survives the harder distribution. Until this, strike_window
            # is floored at strike_window_precision_floor (≥ control dt 0.02s).
            "precision_unlock_v_in_high": 4.0,
            "precision_unlock_hsr": 0.75,
            # Floor on strike_window during stages 0–2. 0.04s = 2 control ticks
            # at 50Hz, leaving the policy enough time to commit to the hit.
            # Once precision unlocks, the window-curriculum tier ladder takes
            # over and can shrink down to 0.01s.
            "strike_window_precision_floor": 0.04,
            # Legacy v→y params left at their defaults; ignored when
            # locomotion_first=True.
    ...
    task_phase = CurrTerm(
        ...
        params={
            ...
            "phase_1_min_iters": 2500, # FROM-SCRATCH: Phase 1 ≥2000 iter — gives the intra-Phase-1 face ramp (posture-first) room to learn distinct fh/bh postures THEN raise the face reward. Orig rationale:

...
State_RLMimic::State_RLMimic(int state_mode, std::string state_string)
    : FSMState(state_mode, state_string)
{
    std::cerr << "[DEBUG] State_RLMimic ctor: " << state_string << std::endl;
    ...
    const auto motion_npz_path = policy_dir / "params" / "G1_Take_102.bvh_60hz.npz";
    env->motion_player = std::make_unique<MimicMotionPlayer>(
        motion_npz_path.string(),

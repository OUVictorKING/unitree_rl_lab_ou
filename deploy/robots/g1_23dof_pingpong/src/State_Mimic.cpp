...
namespace
{
    ...
    {
        const float a = std::clamp(x, 0.0f, 1.0f);
    ...
    : FSMState(state_mode, state_string)
{
    ...
    if (cfg["time_end"])
        ...
        time_range_[1] = motion_->duration;
    switch_blend_s_ = cfg["switch_blend_s"] ? std::max(0.0f, cfg["switch_blend_s"].as<float>()) : switch_blend_s_;
...
void State_Mimic::enter()
{
    ...
    const Eigen::VectorXf target_q = motion->joint_pos();

    {
        std::lock_guard<std::mutex> lock(switch_mtx_);
        switch_start_q_.assign(env->robot->data.joint_pos.data(),
                               env->robot->data.joint_pos.data() + env->robot->data.joint_pos.size());
        switch_target_q_.assign(target_q.data(), target_q.data() + target_q.size());
        if (switch_start_q_.size() != switch_target_q_.size())
            throw std::runtime_error("State_Mimic switch interpolation dim mismatch.");
        switch_start_time_ = std::chrono::steady_clock::now();
        switch_active_ = switch_blend_s_ > 1.0e-4f;
    }
    ...
    policy_thread = std::thread([this]
        ...
        if (switch_blend_s_ > 1.0e-4f)
        {
            const auto blend_duration = std::chrono::duration_cast<clock::duration>(std::chrono::duration<double>(switch_blend_s_));
            const auto blend_end = clock::now() + blend_duration;
            while (policy_thread_running && clock::now() < blend_end)
            {
                std::this_thread::sleep_until(sleepTill);
                sleepTill += dt;
            }
        ...
        while (policy_thread_running)
        {
            ...
            motion->update(env->episode_length * env->step_dt + time_range_[0]);
            env->step();
            if (first_policy_step)
            {
                std::lock_guard<std::mutex> lock(switch_mtx_);
                switch_active_ = false;
                first_policy_step = false;
            }
...
void State_Mimic::run()
{
    {
        std::lock_guard<std::mutex> lock(switch_mtx_);
        if (switch_active_ && !switch_start_q_.empty() && switch_start_q_.size() == switch_target_q_.size())
        {
            const float elapsed = std::chrono::duration<float>(std::chrono::steady_clock::now() - switch_start_time_).count();
            const float alpha = smoothstep(switch_blend_s_ > 1.0e-4f ? elapsed / switch_blend_s_ : 1.0f);
            for (int i = 0; i < env->robot->data.joint_ids_map.size(); ++i)
            {
                const float q = (1.0f - alpha) * switch_start_q_[i] + alpha * switch_target_q_[i];
                lowcmd->msg_.motor_cmd()[env->robot->data.joint_ids_map[i]].q() = q;
            }
            return;
        }
    }

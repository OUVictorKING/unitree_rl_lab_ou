    ...
    : FSMState(state_mode, state_string)
{
    ...
    bad_orientation_grace_s_ = cfg["bad_orientation_grace_s"] ? std::max(0.0f, cfg["bad_orientation_grace_s"].as<float>()) : 0.0f;
    if (bad_orientation_grace_s_ > 0.0f)
        spdlog::info("{} bad-orientation grace: {:.2f}s", state_string, bad_orientation_grace_s_);
            ...
            [&]() -> bool
            {
                if (bad_orientation_grace_s_ > 0.0f)
                {
                    const float elapsed = std::chrono::duration<float>(
                                              ...
                                              .count();
                    if (elapsed < bad_orientation_grace_s_)
                        return false;
                }
...
void State_RLBase::run()
{
    ...
    const bool policy_ready = policy_output_ready_.load();
    ...
    if (!printed_once)
    {
        ...
        for (int i = 0; i < std::min<int>(6, (int)action.size()); ++i)
        {
            ...
            float processed_q_des = action[i];
            float hold_q_des = (entry_hold_q_.size() == action.size()) ? entry_hold_q_[i] : action_offset_[i];

            std::cerr << "  idx=" << i
                      << " joint=" << kActionNames[i]
                      << " sdk=" << sdk_id
                      << " policy_ready=" << policy_ready
                      << " processed_q_des=" << processed_q_des
                      << " hold_q_des=" << hold_q_des
                      << " q=" << q_meas
                      << " dq=" << dq_meas
                      << std::endl;
        }
    ...
    for (int i = 0; i < (int)env->robot->data.joint_ids_map.size(); ++i)
    {
        ...
        if (!policy_ready)
            q_des = (entry_hold_q_.size() == action.size()) ? entry_hold_q_[i] : action_offset_[i];
